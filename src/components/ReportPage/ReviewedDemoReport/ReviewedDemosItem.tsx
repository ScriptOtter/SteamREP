...
import { VerdictReviewedDemos } from "./VerdictReviewedDemos";
import { IReport } from "@/models/IDemos";
import { cn } from "@/lib/utils";
interface IProps {
  report: IReport;
}
export const ReviewedDemosItem = ({ report }: IProps) => {
    ...
    <>
      <div
        ...
        className={cn(
          ...
          " text-white bg-gray grid grid-cols-[1fr_1fr_1fr_1fr_0.5fr] px-2 border-1 hover:border-light-gray-3 duration-150 cursor-pointer my-1 py-4.5"
        )}
      >
        {!isMenuOpen && (
          <>
      ...
      {isMenuOpen && (
        <VerdictReviewedDemos key={report.youtubeLink} report={report} />
      )}
    </>
  );
};
